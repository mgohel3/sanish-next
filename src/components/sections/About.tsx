...
export default function About() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // The pin/grow effect is desktop-only (matches the `lg:` breakpoint
      // the two-column layout already switches on). On phones and tablets
      // pinning a section combined with a scaling full-bleed video fights
      // the browser's dynamic viewport (address bar show/hide changes
      // `innerHeight` mid-scroll), which is what caused the viewport bugs
      // there — so below `lg` the video is left as a normal in-flow
      // element with no ScrollTrigger at all.
  ...
  return (
    ...
    >
      <div className="site-container grid grid-cols-1 items-center gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:gap-20">
        ...
        <div
          ref={mediaRef}
          className="home-media-rectangle about-media-wrap relative shadow-[0_24px_70px_rgba(36,38,43,0.12)]"
          style={{
            aspectRatio: "16 / 10",
