...
import { getApplication, getRelatedApplications, applications } from "@/lib/applications";
...
export default async function ApplicationCaseStudy({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const app = getApplication(slug);
  if (!app) notFound();

  const related = getRelatedApplications(app, 3);
  const accent = CATEGORY_ACCENT[app.category] ?? "#85addc";
            ...
            <div>
              ...
              {app.gallery.length > 0 && (
                <div className="mb-14">
                    ...
                    {app.gallery.slice(1).map((g, i) => (
                      <div key={i} className="overflow-hidden rounded-[20px]" style={{ minHeight: "180px" }}>
                        <img src={g} alt={`${app.label} ${i + 2}`} className="w-full h-full object-cover" />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  {app.productsUsed.map((p) => (
                    <div key={p.name} className="flex flex-col sm:flex-row sm:items-start gap-4 p-6 rounded-[20px] border border-[var(--color-border-subtle)] bg-[var(--bg-secondary)]">
                      ...
                      <div className="w-14 h-14 rounded-xl flex-shrink-0 flex items-center justify-center text-[10px] font-bold text-white uppercase tracking-wider"
                        style={{ backgroundColor: accent, fontFamily: "var(--font-jakarta)" }}>
                        {p.finish.slice(0, 2)}
                      </div>
                      ...
                      {p.productSlug && (
                        <Link href={`/products/${p.productSlug}`}
                          className="flex-shrink-0 text-[11px] font-semibold uppercase tracking-[0.09em] px-4 py-2 rounded-full border transition-all duration-200 hover:-translate-y-[2px] self-start"
        ...
        <section className="home-section--compact border-t border-[var(--color-border-subtle)]"
          ...
          <div className="site-container">
            <div className="flex items-end justify-between mb-10">
              ...
              <Link href="/applications" className="text-[12px] font-semibold hover:opacity-70 transition-opacity"
                style={{ color: accent, fontFamily: "var(--font-jakarta)" }}>
                View All →
              </Link>
            </div>
